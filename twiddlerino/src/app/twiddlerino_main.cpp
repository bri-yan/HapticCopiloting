...
void twiddlerino_setup() {
  ...
  motor1_handle = {
    ...
    .pwm_channel = ledc_channel_t::LEDC_CHANNEL_0,
  ...
  motor2_handle = {
    ...
    .pwm_channel = ledc_channel_t::LEDC_CHANNEL_1,
    .state = motor_state_t::MOTOR_NOT_INITIALIZED,
    .handle_name = "motor2"
    };
...
void TaskPublishTelemetry(void *pvParameters) {
  ...
  for(;;) 
  {
    if(enable_debug_telemetry && (xQueueReceive(xQueueTelemetry, &t, 1) == pdTRUE)){
...
void TaskRunTControl(void *pvParameters){
  ...
  controller1_handle->curr_sens_handle = &current_sens_1_handle;
  ...
  if (ENABLE_SECOND_CONTROLLER) {
    ...
    controller2_handle->curr_sens_handle = &current_sens_2_handle;
