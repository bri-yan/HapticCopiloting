...
static void pid_callback(void *args)
{
    ...
    // Apply admittance law, where the setpoint signal is a position
    // feed position into a position controller 
            setpoint_signal = telem.torque_external/(controller_config.admittance.K) -
            (controller_config.admittance.D/controller_config.K) * (setpoint.vel - telem.filtered_velocity) 

            admittance_torque = telem.torque_external*controller_config.K
            // Feedback signal is position
            feedback_signal = telem.position;
            break;        
        default:
            feedback_signal = telem.position;
...
// Update the admittance control

// Unified Target Admittance and Impedance Control with Transition Factor
    void unifiedControl(double external_torque, double target_torque, double transition_time) {
        double time_init = telem.timestamp_ms
        double impedance_torque; // get from impedance control mode
        double admittance_torque; // admittance control mode
        //transition time -> need to use elapsed time from the start of the transition
        double elapsed_time = telem.timestamp_ms - time_init;
        while (telem.torque_external > 0) {
            // when transition factor hits 0 we have full compliance 
            transition_factor = -1/transition_time*elapsed_time+1
            if (transition_factor == 0) {
                break;
            }
            double blended_torque = (1 - transition_factor) * admittance_torque + transition_factor * impedance_torque;

        }
        while (telem.torque_external == 0) {
            // no torque so we are going into resistive mode with impedance
            // transition factor is increasing until it hits 1
            transition_factor = 1/transition_time*elapsed_time 
        ...
        // Calculate blended torque 
        // if transition factor is 0 then we have compliance - if it's one we have resistive mode

    }

    /* void update(double external_force) {
        position = telem.torque_external/stiffness - damping/stiffness*telem.velocity
    }

void updateControl() {

        if (telem.torque_external < 0.001) {
            transition_factor = 1
            else {
                transition_factor = 0
            }
    ...
    unifiedControl(telem.external_torque, telem.torque_control, transition_factor);
